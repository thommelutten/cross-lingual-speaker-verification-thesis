...
voxceleb1_trials=data/sparc_eval_test/trials
...
if [ $stage -le 0 ]; then
  ...
  path_to_SPARC_dir="SPARC_multiple/$test_to_run";

  echo "Path is: $path_to_SPARC_dir";

  ./make_sparc.pl $path_to_SPARC_dir
  echo "Done making SWAN dataset for $path_to_SPARC_dir";

fi

# if [ $stage -le 1 ]; then
#   # Make MFCCs and compute the energy-based VAD for each dataset
#   for name in sparc_eval_enroll sparc_eval_test; do
#     steps/make_mfcc.sh --write-utt2num-frames true --mfcc-config conf/mfcc.conf --nj 40 --cmd "$train_cmd" \
#       data/${name} exp/make_mfcc $mfccdir
#     #make_rpcc.sh --write-utt2num-frames true --mfcc-config conf/mfcc.conf --nj 40 --cmd "$train_cmd" \
#       data/${name} exp/make_mfcc $mfccdir
...
sid/nnet3/xvector/extract_xvectors.sh --cmd "$train_cmd --mem 6G" --nj 40 \
  $nnet_dir data/sparc_eval_test \
  exp/xvectors_sre16_eval_test

# The SRE16 enroll data
sid/nnet3/xvector/extract_xvectors.sh --cmd "$train_cmd --mem 6G" --nj 40 \
  $nnet_dir data/sparc_eval_enroll \
...
if [ $stage -le 11 ]; then
  ...
  $train_cmd exp/scores/log/sre16_eval_scoring.log \
    ...
    "ivector-copy-plda --smoothing=0.0 $nnet_dir/xvectors_train/plda - |" \
    "ark:ivector-mean ark:data/sparc_eval_enroll/spk2utt scp:exp/xvectors_sre16_eval_enroll/xvector.scp ark:- | transform-vec $nnet_dir/xvectors_train/transform.mat ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
    "ark:ivector-subtract-global-mean $nnet_dir/xvectors_train/mean.vec scp:exp/xvectors_sre16_eval_test/xvector.scp ark:- | transform-vec $nnet_dir/xvectors_train/transform.mat ark:- ark:- | ivector-normalize-length ark:- ark:- |" \
    "cat '$voxceleb1_trials' | cut -d\  --fields=1,2 |" exp/scores_voxceleb1_test || exit 1;
    # ark:ivector-subtract-global-mean $nnet_dir/xvectors_train/mean.vec ark:- ark:- |
fi
